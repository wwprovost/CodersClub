...
/*
 * Modified to remove the slf4j dependency -- just using java.util.logging instead.
 */
public final class SimpleSmtpServer implements AutoCloseable {

	private static final Logger LOG = 
			Logger.getLogger(SimpleSmtpServer.class.getName());
	...
	public void stop() {
		...
		try {
			...
			LOG.log(Level.WARNING, "trouble closing the server socket", e);
		}
		// and block until worker is finished
		try {
			workerThread.join(STOP_TIMEOUT);
		} catch (InterruptedException e) {
			LOG.log(Level.WARNING, "interrupted when waiting for worker thread to finish", e);
		}
	}
	...
	private void performWork() {
		try {
			...
			if (!stopped) {
				LOG.log(Level.SEVERE, "hit exception when running server", e);
				try {
					serverSocket.close();
				} catch (IOException ex) {
					LOG.log(Level.SEVERE, "and one when closing the port", ex);
				}
			}
		}
	}
